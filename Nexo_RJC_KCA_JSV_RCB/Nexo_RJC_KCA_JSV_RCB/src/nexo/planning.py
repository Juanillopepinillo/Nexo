...
def district_capacity_table(
    ...
    for reuse_factor in scenario.reuse_factors:
        ...
        omnidirectional_sir = estimated_sir_db(reuse_factor, scenario.path_loss_exponent, interferers=6)
        ...
        rows.append(
            {
                ...
                "sectors_per_site": scenario.sectors_per_site,
                ...
                "omnidirectional_sir_db": omnidirectional_sir,
                "sectorization_gain_db": sectorized_sir - omnidirectional_sir,
                ...
                "sectors_for_target_area": site_count_for_area(scenario.target_area_km2, design_area) * scenario.sectors_per_site,
            }
        )
...
def compliance_table(
    ...
    rows = [
        (
            ...
            "Incluido en la web y en el informe tecnico",
            "Si",
            "Se incorporan contexto LTE/5G, OFDMA, MCS, propagacion y capacidad.",
        ),
        (
            "Metodologia reproducible",
            "Ruido, sensibilidad, Lmax, cobertura, trafico, Erlang B y radio final",
            "Si",
            "La secuencia de calculo sigue el orden exacto marcado en el enunciado.",
        ),
        (
            "Escenario A con reutilizacion y sectorizacion",
            f"N={int(district_choice['reuse_factor_n'])}, {int(district_choice['sectors_per_site'])} sectores y comparacion frente a omni",
            "Si",
            "Se cuantifican SIR sectorizada, SIR omni y ganancia por sectorizacion.",
        ),
        (
            "Escenario B con cell splitting",
            f"Primera etapa viable: S{int(festival_choice['split_stage'])}",
            "Si",
            "Se evalua la secuencia completa de splitting hasta satisfacer la demanda.",
        ),
        (
            "Comparacion cobertura frente a capacidad",
            f"A: {float(district_choice['design_radius_km']):.3f} km; B: {float(festival_cap['design_radius_km']):.3f} km",
            "Si",
            "Ambos escenarios muestran el criterio limitante y el radio final de diseno.",
        ),
        (
            "Numero de celdas o sitios en area objetivo",
            f"A: {int(district_choice['sites_for_target_area'])} sitios/1 km2; B: {int(festival_choice['sites_for_target_area'])} celdas/1 km2",
            "Si",
            "El area objetivo se normaliza a 1 km2 por no venir fijada en la guia docente.",
        ),
        (
            "Figura resumen final",
            "Incluida en la web y en assets/figures",
            "Si",
            "Sintetiza las decisiones de diseno para ambos escenarios.",
        ),
    ]
    return pd.DataFrame(rows, columns=["bloque_enunciado", "evidencia", "cumple", "comentario"])


def numerical_validation_table(
    ...
    district_n3 = district_capacity.loc[district_capacity["reuse_factor_n"] == 3].iloc[0]
    district_n4 = district_capacity.loc[district_capacity["reuse_factor_n"] == 4].iloc[0]
    festival_cov = festival_coverage.iloc[0]
    festival_cap = festival_capacity.iloc[0]
    festival_s3 = festival_splitting.loc[festival_splitting["split_stage"] == 3].iloc[0]
    festival_s4 = festival_splitting.loc[festival_splitting["split_stage"] == 4].iloc[0]

    rows = [
        (
            "Ruido termico comun",
            f"{float(district_cov['thermal_noise_dbm']):.3f} dBm",
            "-93.990 dBm aproximados para 20 MHz y NF=7 dB",
            "Si",
        ),
        (
            "Sensibilidad escenario A",
            f"{float(district_cov['receiver_sensitivity_dbm']):.3f} dBm",
            "N + 15 dB + 2 dB",
            "Si",
        ),
        (
            "Sensibilidad escenario B",
            f"{float(festival_cov['receiver_sensitivity_dbm']):.3f} dBm",
            "N + 5 dB + 2 dB",
            "Si",
        ),
        (
            "Factibilidad radio N=3",
            f"Margen SIR = {float(district_n3['sir_margin_db']):.3f} dB",
            "Debe ser negativo para descartar N=3",
            "Si" if float(district_n3["sir_margin_db"]) < 0.0 else "No",
        ),
        (
            "Factibilidad radio N=4",
            f"Margen SIR = {float(district_n4['sir_margin_db']):.3f} dB",
            "Debe ser positivo para aceptar N=4",
            "Si" if float(district_n4["sir_margin_db"]) > 0.0 else "No",
        ),
        (
            "Factor limitante en A",
            str(district_n4["limiting_factor"]),
            "Debe dominar capacidad",
            "Si" if str(district_n4["limiting_factor"]) == "capacity" else "No",
        ),
        (
            "Factor limitante en B",
            str(festival_cap["limiting_factor"]),
            "Debe dominar capacidad",
            "Si" if str(festival_cap["limiting_factor"]) == "capacity" else "No",
        ),
        (
            "Split S3",
            f"Cumple demanda = {str(bool(festival_s3['meets_demand']))}",
            "Debe ser insuficiente",
            "Si" if not bool(festival_s3["meets_demand"]) else "No",
        ),
        (
            "Split S4",
            f"Cumple demanda = {str(bool(festival_s4['meets_demand']))}",
            "Debe ser la primera etapa viable",
            "Si" if bool(festival_s4["meets_demand"]) else "No",
        ),
    ]
    return pd.DataFrame(rows, columns=["control", "resultado", "criterio", "valido"])
